...
namespace BianCore.Modules.Minecraft
{
    public class Launcher
    {
        ...
        {
            // JVM 参数
            StringBuilder jvmSb = new StringBuilder('\"' + prop.JVMProperties.JVMPath + '\"');
                ...
                foreach (var arg in prop.LaunchVersion.Arguments.Value.JVM)
                {
                    ...
                    foreach (var rule in arg.Rules)
                    {
                        if (rule.OS_Name != null
                            && rule.OS_Name != SystemTools.GetOSPlatform().ToString().ToLower())
                        {
                            allow = !rule.IsAllow;
                            if (!allow) break;
                        }
                        if (rule.OS_Arch != null
                            && rule.OS_Arch != SystemTools.GetArchitecture().ToString().ToLower())
                        {
                            allow = !rule.IsAllow;
                            if (!allow) break;
                        }
            ...
            else // 1.12 及以下版本参数（不包含 JVM 参数）
            {
                if (SystemTools.GetOSPlatform() == SystemTools.OSPlatform.Windows)
                {
                    ...
                    jvmSb.Append(" -Dos.name=\"Windows 10\"");
                    jvmSb.Append(" -Dos.version=10.0");
                }
                else if (SystemTools.GetOSPlatform() == SystemTools.OSPlatform.MacOS)
                {
            ...
            libStrs.Add(Path.Combine(prop.LaunchVersion.VersionPath, $"{prop.LaunchVersion.ID}.json"));
            jvmSb.Replace("${classpath}", '\"'
                + string.Join(Config.ClassPath_Separator.ToString()
                , libStrs) + '\"');
            ...
            return jvmSb.Append(gameSb.ToString()).ToString();
        }
        ...
        public LibraryStruct[] GetLibraries(VersionInfo ver)
        {
            ...
            foreach (var lib in ver.Libraries)
            {
                ...
                {
                    if (rule.OSName != null
                        && rule.OSName != SystemTools.GetOSPlatform().ToString().ToLower())
                    {
                        allow = !rule.IsAllow;
                        if (!allow) break;
                    }
        ...
        public string[] LibrariesToPaths(LibraryStruct[] libs)
        {
            ...
            {
                string[] name = libs[i].Name.Split(':');
                name[0] = name[0].Replace('.', Path.PathSeparator);
                result[i] = Path.Combine(MinecraftPath, "libraries"
                    , string.Join(Path.PathSeparator.ToString(), name)
